...
def test_event_filter_by_free_seats(event):
    events = Event.objects.annotate(booked=Count('bookings')).filter(booked__lt=F('seats'))
    assert len(events) > 0
    assert event in events
...
def test_event_filter_by_booked_seats(booked_event):
    events = Event.objects.annotate(booked=Count('bookings')).filter(booked__gte=F('seats'))
    assert len(events) == 0
    events = Event.objects.annotate(booked=Count('bookings')).filter(booked__lt=F('seats'))
...
def test_event_filter_by_rating(rated_event, user):
    events = Event.objects.annotate(avg_rating=Avg('ratings__score')).filter(avg_rating__gte=3)
    assert len(events) > 0
